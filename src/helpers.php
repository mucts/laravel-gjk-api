...
if (!function_exists('gjk_request')) {
    ...
    {
        $params = array_merge(["app_id" => config('gjk.app_id'), "user_id" => mt_rand(1000000, 3000000), "c_ver" => config('gjk.secret_key'), "nonce_str" => str_random(mt_rand(6, 32))], $params);
        $params = array_merge($params, ["sign" => Sign::makeSign($params, config('gjk.secret_key'))]);
        $url = trim('gjk.url', '/') . '/' . ltrim($route, '/');
